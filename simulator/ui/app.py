...
SESSION_ID = "default"  # single-user tool for now
...
def _chain_df(candidates) -> pd.DataFrame:
    ...
    return candidates if isinstance(candidates, pd.DataFrame) else pd.DataFrame(candidates)
...
# Pagination settings for the "Browse chain" tables.
CHAIN_PAGE_SIZE = 10
# How many contracts to ask view_chain() for, before client-side filtering.
CHAIN_FETCH_LIMIT = 60
...
def _render_leg_picker(session: TradingSession, snap: dict, side: str,
                        fetch_max_expiry: int, fetch_min_expiry: int):
    """
    Filter -> paginate -> select -> open flow for one leg ("long" or "short").
    Renders the Browse chain expander and the Open leg button beneath it.
    """
    full = _chain_df(session.view_chain(max_expiry_days=fetch_max_expiry,
                                         min_expiry_days=fetch_min_expiry,
                                         limit=CHAIN_FETCH_LIMIT))
    if "strike" in full.columns:
        full = full.sort_values("strike").reset_index(drop=True)
    ...
    with st.expander("Browse chain", expanded=True):
        if len(full):
            ...
            with fc1:
                strike_lo, strike_hi = float(full["strike"].min()), float(full["strike"].max())
                strike_range = st.slider("Strike range", strike_lo, strike_hi,
                                          (strike_lo, strike_hi), key=f"{side}_strike_filter")
            with fc2:
                expiry_options = sorted(full["expiry"].unique())
                selected_expiries = st.multiselect("Expiry", expiry_options,
                                                    default=expiry_options, key=f"{side}_expiry_filter")
            with fc3:
                delta_lo, delta_hi = float(full["delta"].min()), float(full["delta"].max())
                delta_range = st.slider("Delta range", 0.0, 1.0,
                                         (delta_lo, delta_hi), key=f"{side}_delta_filter")

            filtered = full[
                (full["strike"].between(*strike_range)) &
                (full["expiry"].isin(selected_expiries)) &
                (full["delta"].between(*delta_range))
...
with long_col:
    ...
    if snap["long_leg"] is None:
        _render_leg_picker(session, snap, side="long", fetch_max_expiry=400, fetch_min_expiry=1)
    else:
        leg = snap["long_leg"]
...
with short_col:
    ...
    if snap["short_leg"] is None:
        _render_leg_picker(session, snap, side="short", fetch_max_expiry=60, fetch_min_expiry=1)
    else:
        leg = snap["short_leg"]
        st.write(f"**${leg['strike']} call** \u00b7 exp {leg['expiry']} \u00b7 entry ${leg['entry_price']:.2f}")
